...
// Template Família (2) — "Cantinho da Família". Visual acolhedor (terracota) e centralizado.
export function CantinhoDaFamilia({ content, photos }: TemplateProps) {
  ...
  return (
    <div className="min-h-screen bg-orange-50 text-stone-800">
      <header className="px-6 py-16 text-center">
        <p className="text-sm uppercase tracking-[0.25em] text-orange-600">Cantinho da família</p>
      ...
      <div className="mx-auto max-w-2xl px-5 pb-16">
        {cover && (
          <BlurFade>
            <div className="rounded-[2rem] border border-orange-100 bg-white p-2 shadow-sm">
            ...
            <section className="mt-12">
              ...
              <div className="mt-6 grid gap-3 sm:grid-cols-2">
                {content.moments.map((m) => (
                  <div key={m.id} className="rounded-2xl border border-orange-100 bg-white p-4">
                    {m.date && <p className="text-sm text-orange-600">{m.date}</p>}
        ...
        {content.finalPhrase && (
          <BlurFade>
            <section className="mt-12 text-center">
              <p className="font-serif text-2xl italic text-orange-700">{content.finalPhrase}</p>
            </section>
          </BlurFade>
        )}
      </div>
